...
def preprocess(img, demo=False):
    ...
    # FEATURE 3: Canny features
    canny_features = canny(gray, sigma = 2)
        
    if demo:
        canny_image = canny_features #canny_features, 
        axes_idx = _demo_plot(canny_image, stage="CANNY features", axes_idx=axes_idx)
    
    # return a flat array of the raw, hog and canny features
    return np.hstack([color_info, hog_features, canny_features.flatten()])